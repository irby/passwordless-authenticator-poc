...
export class AdminUserListModalComponent implements OnInit {
      ...
      switch (toggleResp.statusCode) {
        case HttpStatusCode.Conflict:
          this.notificationService.error(
            "You know you can't deactivate yourself, right?"
          );
          break;
        default:
          this.notificationService.error(
            "An error occurred toggling user status"
          );
      }
  ...
  public async deactivateGrants(user: UserDto) {
    const deactivationResponse =
      await this.adminService.deactivateGrantsForUser(user.id);
    if (deactivationResponse.type !== "data") {
      this.notificationService.error(
        "An error occurred while deactivating grants for user"
      );
      return;
    }
    this.notificationService.success(
      "Grants successfully deactivated for user"
    );
  }
