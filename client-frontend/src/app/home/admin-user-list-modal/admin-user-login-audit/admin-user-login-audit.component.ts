...
export class AdminUserLoginAuditComponent implements OnInit {
  ...
  constructor(
    @Inject(MAT_DIALOG_DATA) private readonly user: UserModalInfo,
  ...
  async ngOnInit() {
    ...
    this.isLoading = true;
    const auditLogResp = await this.adminService.getLoginAuditLogForUser(
      this.user.userId
    );
    ...
    if (auditLogResp.type !== "data") {
      this.notificationService.error(
        "failed to retrieve login audits for user"
      );
      return;
    }
