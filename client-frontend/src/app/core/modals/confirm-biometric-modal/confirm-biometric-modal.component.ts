...
import {
  GenerateWebAuthnLoginFinalizeRequest,
  WebAuthnLoginFinalizeRequest,
} from "../../models/webauthn/webauthn-login-finalize-request.interface";
import {
  CreateAccountWithGrantResponse,
  GrantAttestationObject,
  WebauthnLoginInitializeResponse,
} from "../../models/webauthn/webauthn-login-initialize-response.interface";
import { AuthenticationService } from "../../services/authentication.service";
import {
  ChallengeService,
  SignChallengeAsUserResponse,
} from "../../services/challenge.service";
import {
  BeginCreateAccountWithGrantRequest,
  FinishCreateAccountWithGrantRequest,
  GrantService,
...
export class ConfirmBiometricModalComponent implements OnInit {
  ...
  constructor(
    private readonly dialogRef: MatDialogRef<ConfirmBiometricModalComponent>,
  ...
  public async submitBiometric(isGood: boolean) {
    ...
    if (await this.submitSignature(challengeResp)) {
      ...
      this.dialogRef.close({
        data: { isSuccess: true } as ConfirmBiometricModalData,
      });
    }
  }
  ...
  private async signChallenge(
    ...
    challenge: string,
    isGood: boolean
  ): Promise<WebAuthnLoginFinalizeRequest | null> {
    const sanitizedChallenge =
      ChallengeSanitizationUtil.sanitizeInput(challenge);
    const signResp = await this.challengeService.signChallenge(
      userId,
      sanitizedChallenge
    );
    ...
    if (signResp.type !== "data") {
      this.notificationService.error(
        "An error occurred signing challenge",
        "Error"
      );
  ...
  private generateRequestModel(
    data: SignChallengeAsUserResponse,
    isGood: boolean
  ): WebAuthnLoginFinalizeRequest {
  ...
  private async submitSignature(
    finalizeRequest: InitializeChallengeResponse
  ): Promise<boolean> {
  ...
  private async initializeRequest(
    data: ConfirmBiometricData,
    isGood: boolean
  ): Promise<InitializeChallengeResponse | null> {
    switch (data.context) {
      case ConfirmBiometricContext.Login:
        ...
        if (loginResp.type !== "data") {
          this.notificationService.error(
            "An error occurred fetching challenge",
            "Error"
          );
          this.isLoading = false;
          return null;
        ...
        const signedChallenge = await this.signChallenge(
          this.data.userId,
          loginResp.data.publicKey.challenge,
          isGood
        );

        return { signature: signedChallenge };
      case ConfirmBiometricContext.UserConfirmGrant:
        const grantResp = await this.initializeCreateAccountWithGrant(
          data.guestUserId ?? "",
          data.grantId ?? ""
        );
        ...
        const signature = await this.signChallenge(
          this.data.userId,
          grantResp.data.options.publicKey.challenge,
          isGood
        );
        const attestation = await this.signChallenge(
          this.data.userId,
          this.createAttestationObject(grantResp.data.grant),
          isGood
        );

        return {
          signature: signature,
          attestation: attestation?.response.signature,
        };
      default:
  ...
  private async initializeWebauthnLogin(
    userId: string
  ): Promise<ServiceResponse<WebauthnLoginInitializeResponse>> {
    return await this.authenticationService.beginWebauthnLogin(userId);
  }

  private async initializeCreateAccountWithGrant(
    guestUserId: string,
    grantId: string
  ...
  private async finalizeRequest(
    ...
    switch (data.context) {
      ...
      case ConfirmBiometricContext.UserConfirmGrant:
        ...
        return await this.finalizeCreateAccountWithGrant(
          data.guestUserId ?? "",
          data.grantId ?? "",
          finalizeRequest.attestation,
          finalizeRequest.signature
        );
      default:
        throw new Error(`unknown context ${data.context}`);
    }
  ...
  private async finalizeWebauthnLogin(
    finalizeRequest: WebAuthnLoginFinalizeRequest
  ): Promise<ServiceResponse<any>> {
    return await this.authenticationService.finalizeWebauthnLogin(
      finalizeRequest
    );
  }

  private async finalizeCreateAccountWithGrant(
    guestUserId: string,
    grantId: string,
  ...
  private createAttestationObject(
