...
export class GrantService extends BaseService {
    ...
    public async finishCreateAccountWithGrant(request: FinishCreateAccountWithGrantRequest): Promise<ServiceResponse<any>> {
        return await this.postAsync(`access/share/finish-create-account-with-grant`, request);
    }
...
export interface FinishCreateAccountWithGrantRequest extends WebAuthnLoginFinalizeRequest {
    guestUserId: string;
    grantId: string;
